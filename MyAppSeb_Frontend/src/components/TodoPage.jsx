...
export default function TodoPage() {
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden text-white relative flex flex-col p-4 sm:p-6 select-none">
      ...
      {/* Fondo Persona 5 Ciudad Responsivo */}
      <div 
      ...
      {/* Contenido Principal */}
      <main className="w-full max-w-4xl mx-auto flex flex-col gap-4 sm:gap-6 z-10 my-auto">
        
        {/* Formulario (Responsivo en móviles) */}
        <form onSubmit={handleAddTask} className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full transform -rotate-1">
          <div className="relative flex-1">
            <input
              ...
              className="w-full bg-black text-white placeholder-neutral-500 font-bold italic tracking-wider px-3 sm:px-5 py-2.5 sm:py-3 border-2 sm:border-3 border-neutral-700 focus:border-red-600 focus:outline-none shadow-[4px_4px_0px_0px_rgba(220,38,38,1)] text-sm sm:text-lg"
            />
          </div>
          <button
            type="submit"
            className="bg-black hover:bg-neutral-900 text-white border-2 sm:border-3 border-white px-6 sm:px-8 py-2.5 sm:py-3 font-black italic tracking-widest uppercase shadow-[4px_4px_0px_0px_rgba(220,38,38,1)] hover:translate-x-0.5 hover:translate-y-0.5 transition-all text-sm sm:text-lg font-sans"
          >
            AÑADIR
          </button>
        </form>
        ...
        {/* Lista de Tareas */}
        {loading ? (
          <div className="text-center font-black italic text-yellow-400 text-lg sm:text-xl tracking-widest py-10 drop-shadow-[2px_2px_0px_rgba(0,0,0,1)]">
            CARGANDO TAREAS...
          </div>
        ) : (
          <div className="flex flex-col gap-3 sm:gap-4 mt-2 sm:mt-4 max-h-[500px] overflow-y-auto pr-1">
            {tasks.length === 0 ? (
              ...
              tasks.map((task, index) => {
                ...
                return (
                  <div 
                    ...
                    className={`group relative cursor-pointer transform transition-all duration-200 ${rotationClass} hover:scale-[1.01] hover:z-20`}
                  >
                    <div className="absolute inset-0 bg-black translate-x-1.5 sm:translate-x-2 translate-y-1.5 sm:translate-y-2 -z-10" />

                    <div 
                      className={`w-full border-2 border-neutral-800 p-3 sm:p-4 flex items-center justify-between gap-3 transition-colors ${
                    ...
                    >
                      <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
                        <div className={`w-6 h-6 sm:w-8 sm:h-8 border-2 flex items-center justify-center font-black text-sm sm:text-xl italic shrink-0 ${
                          isCompleted 
                        ...
                        <span className={`text-sm sm:text-lg font-black italic tracking-wide font-sans truncate ${
                      ...
                      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                        <div className={`px-2 sm:px-3 py-0.5 sm:py-1 font-mono text-[10px] sm:text-xs font-bold tracking-widest italic uppercase transform -skew-x-12 ${
                          isCompleted 
